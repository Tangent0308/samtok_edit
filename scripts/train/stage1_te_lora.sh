...
PYTHONPATH="$DIFFSYNTH_DIR:${PYTHONPATH:-}" accelerate launch \
  ...
  --max_pixels 1048576 --dataset_repeat "${DATASET_REPEAT:-1}" --dataset_num_workers "${DATASET_WORKERS:-8}" \
  ...
  --learning_rate 4e-5 --weight_decay 0.05 --max_grad_norm 1.0 --warmup_ratio 0.05 --num_epochs 1 \
